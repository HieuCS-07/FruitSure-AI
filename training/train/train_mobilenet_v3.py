"""
Training script SỬ DỤNG MobileNetV3-Large (Model so sánh)
- Chỉ huấn luyện 2 lớp: Fake_Chemical và Real
- Lưu kết quả vào /models_mobilenet và /results_mobilenet
"""
...
# Data augmentation và preprocessing (Giữ nguyên như ResNet để so sánh)
...
def create_model():
    """Tạo model MobileNetV3-Large với transfer learning"""
    
    # --- THAY ĐỔI: Sử dụng MobileNetV3 ---
    ...
    # --- PHẦN SỬA LỖI mat1 and mat2 shapes cannot be multiplied ---
    
    # Kích thước features đầu ra chính xác trước lớp Classifier là 960
    num_features = 960 
    
    # Tái cấu trúc lớp Classifier mới
    model.classifier = nn.Sequential(
        # Lớp Linear đầu tiên (960 -> 512)
        nn.Linear(num_features, 512),
        nn.ReLU(inplace=True),
        nn.Dropout(0.3),
        # Lớp Linear cuối cùng (512 -> 2)
...
def plot_confusion_matrix(y_true, y_pred, save_path='results_mobilenet/confusion_matrix.png'):
    """Vẽ confusion matrix"""
    ...
    plt.title('Confusion Matrix (MobileNetV3 - 2 Lớp)', fontsize=16, fontweight='bold')
...
def plot_training_history(train_losses, val_losses, train_accs, val_accs, 
                          save_path='results_mobilenet/training_history.png'):
    ...
    ax1.set_title('Training & Validation Loss (MobileNetV3)', fontsize=14, fontweight='bold')
    ax1.legend()
    ...
    ax2.set_title('Training & Validation Accuracy (MobileNetV3)', fontsize=14, fontweight='bold')
    ax2.legend()
...
def visualize_predictions(model, data_loader, device, class_names, num_images=5):
    ...
    plt.tight_layout()
    plt.savefig('results_mobilenet/sample_predictions.png', dpi=300, bbox_inches='tight')
    print("Da luu sample predictions: results_mobilenet/sample_predictions.png")
    plt.close()
...
def main():
    """Ham main"""
    print("="*60)
    print("STARTING MODEL TRAINING (MobileNetV3 - 2 LỚP)")
    print("="*60)
    
    # !!! QUAN TRỌNG: Đảm bảo đường dẫn này đúng !!!
    DATA_ROOT = r"C:\Users\MY GEAR\DL\DL\dataset\split_dataset" # <-- SỬA ĐƯỜNG DẪN NÀY
    ...
    print("\nKhoi tao model MobileNetV3...")
    model = create_model() 
    
    # --- CẬP NHẬT: OPTIMIZER CHO MOBILE-NET ---
    # Chỉ tối ưu hóa các tham số của lớp classifier (đã được unfreeze)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            os.makedirs('models_mobilenet', exist_ok=True) 
            torch.save(model.state_dict(), 'models_mobilenet/best_model.pth')
    ...
    model.load_state_dict(torch.load('models_mobilenet/best_model.pth')) 
    test_loss, test_acc, test_f1, test_labels, test_preds = validate(model, test_loader, criterion)
    
    print("\n" + "="*60)
    print("KET QUA CUOI CUNG TREN TEST SET (MobileNetV3)")
    print("="*60)
    ...
    with open('results_mobilenet/metrics.json', 'w') as f:
        json.dump(metrics, f, indent=4)
    print(f"\nDa luu metrics: results_mobilenet/metrics.json") 
    
    print("\nVisualizing random predictions...")
    visualize_predictions(model, test_loader, device, CLASSES, num_images=5)
    
    print("\n" + "="*60)
    print("HOAN THÀNH HUẤN LUYỆN MOBILE-NET!")
    print("="*60)
