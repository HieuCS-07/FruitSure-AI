...
def main():
    ...
    model = create_model() # <-- Giờ sẽ tự động tạo model 2 lớp
        ...
        # Sau khi load train_dataset
    labels = [label for _, label in train_dataset.samples]
    num_chemical = labels.count(train_dataset.class_to_idx['Chemical'])
    num_real = len(labels) - num_chemical
    weight_chemical = (num_real / num_chemical) * 0.45   # ~1.18–1.25 tùy dataset của bạn

    class_weights = torch.tensor([weight_chemical, 1.0], dtype=torch.float).to(device)
