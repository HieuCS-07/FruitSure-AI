"""
Training script (Phiên bản V3 - Đã sửa cho 2 LỚP)
...
# --- THAY ĐỔI QUAN TRỌNG ---
# Classes: 2 loại
# Sắp xếp theo alphabet để ImageFolder nhận diện đúng
CLASSES = ['Chemical', 'Real'] 
NUM_CLASSES = len(CLASSES) # <-- Tự động cập nhật thành 2
# --- KẾT THÚC THAY ĐỔI ---
...
# TẮT TẤT CẢ AUGMENTATION – chỉ Resize + Normalize
...
def create_model():
    weights = models.ResNet50_Weights.IMAGENET1K_V2   # Vẫn dùng pretrained
...
def plot_confusion_matrix(y_true, y_pred, save_path='total/resultsv_no_aug/confusion_matrix.png'):
    """Vẽ confusion matrix"""
...
def plot_training_history(train_losses, val_losses, train_accs, val_accs, 
                          save_path='total/resultsv_no_aug/training_history.png'):
...
def visualize_predictions(model, data_loader, device, class_names, num_images=5):
    ...
    plt.tight_layout()
    plt.savefig('total/resultsv_no_aug/sample_predictions.png', dpi=300, bbox_inches='tight')
    print("Da luu sample predictions: total/resultsv_no_aug/sample_predictions.png")
    plt.close()
...
def main():
    ...
    print("\nLoading datasets...")
    try:
        train_dataset = datasets.ImageFolder(TRAIN_DIR, transform=train_transform_no_aug)
        ...
        # Cập nhật CLASSES global để khớp với những gì ImageFolder tìm thấy
        # (Phòng trường hợp ImageFolder sắp xếp 'Real' trước 'Fake_Chemical')
        CLASSES = train_dataset.classes
        
        # Kiểm tra lại xem số lượng lớp có đúng là 2 không
    ...
    model = create_model() # <-- Giờ sẽ tự động tạo model 2 lớp
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            os.makedirs('total/modelsv_no_aug', exist_ok=True) 
            torch.save(model.state_dict(), 'total/modelsv_no_aug/best_model.pth')
            print(f"   ==> Saved best model! (Val Acc: {val_acc:.2f}%)")
    
    print("\nVe bieu do training history...")
    os.makedirs('total/resultsv_no_aug', exist_ok=True) 
    plot_training_history(train_losses, val_losses, train_accs, val_accs)
    
    print("\nDanh gia tren test set (sử dụng model tốt nhất)...")
    model.load_state_dict(torch.load('total/modelsv_no_aug/best_model.pth')) 
    ...
    with open('total/resultsv_no_aug/metrics.json', 'w') as f:
        json.dump(metrics, f, indent=4)
    print(f"\nDa luu metrics: total/resultsv_no_aug/metrics.json") 
    ...
    print("="*60)
    print("\nCac file ket qua:")
    print("  - total/modelsv_no_aug/best_model.pth")
    print("  - total/resultsv_no_aug/confusion_matrix.png")
    print("  - total/resultsv_no_aug/training_history.png")
    print("  - total/resultsv_no_aug/sample_predictions.png")
    print("  - total/resultsv_no_aug/metrics.json")
